...
from flask_bcrypt import Bcrypt                                                                                     #for encryption of password
...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://root:$Charu6602@localhost/DBMSP'    
#app.config["SQLALCHEMY_TRACK_MODIFICATIONS"] = False                     #database configuration including mysql username,password, and the name of the database
...
class User(db.Model, UserMixin):
    ...
    Country=db.Column(db.String(15),server_default='India')                                                                      #user table in the users database including id, username, and password
...
class RegisterForm(FlaskForm):            
    ...
    Country = StringField(Optional(strip_whitespace=True), validators=[
                            Length(min=2, max=30)], render_kw={"placeholder": "Country"})
    Age = IntegerField(validators=[InputRequired(),validators.NumberRange(min=18, max=1000)], render_kw={"placeholder": "Age"})
...
class ReviewForm(FlaskForm):
    ...
    Book_ID=IntegerField(render_kw={"placeholder":"Enter the ID of the book you chose!"})

    Movie_ID=IntegerField(render_kw={"placeholder":"Enter the ID of the movie you chose!"})

    Doc_ID=IntegerField(render_kw={"placeholder":"Enter the ID of the documentary you chose!"})
...
def reviewstat():
    ...
    data4=curs.fetchall()
    return render_template('reviewstat.html',data=data,data2=data2,data3=data3,data4=data4)
